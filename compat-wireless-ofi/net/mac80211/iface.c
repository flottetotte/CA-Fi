...
{
	//printk(KERN_DEBUG "ieee80211_idle_on");
	if (local->hw.conf.flags & IEEE80211_CONF_IDLE)
		return 0;
...
{
	//printk(KERN_DEBUG "__ieee80211_recalc_idle");
	struct ieee80211_sub_if_data *sdata;
	int count = 0;
	...
	if (!count)
		/*Let's try this - ADDED BY TORSTEN */
...
{
	printk(KERN_DEBUG "ofi init with interface init\n");
	ofi_init();
