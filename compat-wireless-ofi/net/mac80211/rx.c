...
__ieee80211_data_to_8023(struct ieee80211_rx_data *rx, bool *port_control)
{
	...
	if(hdr->frame_control == IEEE_OFI_SYTPE){
		//printk(KERN_DEBUG "ofi got frame\n");
		ofi_msg_handler(rx);
		/*Only because we want the driver to handle the skb stuff*/
		return -1;
	}
...
static bool ieee80211_frame_allowed(struct ieee80211_rx_data *rx, __le16 fc)
{
	...
	/*ADDED BY TORSTEN
	*
	* TODO: need check like has ofi header		
	*
	*/	
...
ieee80211_deliver_skb(struct ieee80211_rx_data *rx)
{
	...
	if (skb) {
		...
		if (skb) {
			...
			//printk(KERN_DEBUG "ieee80211_skb_deliver to local stack");
				...
				struct ieee80211_hdr *hdr)
{
	...
	case NL80211_IFTYPE_STATION:
		...
		if (!multicast && !ether_addr_equal(sdata->vif.addr, hdr->addr1)) {
			if (!(sdata->dev->flags & IFF_PROMISC) || sdata->u.mgd.use_4addr){
				return 0;
			}
			status->rx_flags &= ~IEEE80211_RX_RA_MATCH;
		}
